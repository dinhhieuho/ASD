...
{
	CarShiftContext mycar = new CarShiftContext();
	ShiftState currShift = mycar.getState();
	public Lab5()
	{
		...
		JTextFieldShift.setText(currShift.toString());
		JTextFieldSpeed.setText("0");
	...
	void JSlider1_stateChanged(javax.swing.event.ChangeEvent event)
	{
        ...
        if (!source.getValueIsAdjusting()) {
	        ...
	        mycar.getState().changeState(mycar, mph);
	        //int shift= mycar.changeSpeed(mph);
	        JTextFieldSpeed.setText(String.valueOf(mph));
	        JTextFieldShift.setText(mycar.getState().toString());
	        //if (shift >0)
