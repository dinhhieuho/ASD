...
public class ScribbleCanvasListener 
  ...
  private Tool getTool(int n) {
	  Tool tool = drawframe.getTool(0);;
	    switch (n){	        
	        case 1:  // handle mouse pressed for line tool
	        	tool = drawframe.getTool(1);
	        	tool.setShapeType(0);        	
  ...
  public void mousePressed(MouseEvent e) {
    Point p = e.getPoint(); 
    int current = drawframe.getCurrentTool();
    Tool tool = getTool(current);
    /*switch (current){
        case 0: // handle mouse pressed for scribble tool        
        	tool = drawframe.getTool(0);
        	//tool.mousePressed(canvas, p);
  ...
  public void mouseReleased(MouseEvent e) {
    Point p = e.getPoint(); 
    int current = drawframe.getCurrentTool();
    //Tool tool = drawframe.getTool(current);
    Tool tool = getTool(current);
    /*switch (current){
